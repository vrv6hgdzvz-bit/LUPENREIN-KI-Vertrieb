...
export async function POST(req:Request){
 ...
 const validUntil=body.validUntil||new Date(Date.now()+14*86400000).toISOString().slice(0,10);const offer=await createOffer({leadId:lead.id,surveyId:body.surveyId?String(body.surveyId):undefined,title:String(body.title||`Reinigungsangebot für ${lead.company}`),validUntil,items,notes:String(body.notes||'')});await updateLead(lead.id,{status:'Angebot'});await createActivity({leadId:lead.id,type:'Angebot',direction:'intern',content:`Angebot ${offer.number} erstellt: ${offer.title}`,outcome:'Entwurf'});return NextResponse.json({offer},{status:201})
